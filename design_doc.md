...
> FeatureStore 流引擎 · Arrow + RocksDB 当前实现基线
> 版本：v2.1 · 状态：Implemented
...
```
DropTable(table_id) 流程（当前实现）：
1. 在 __meta__ CF 中标记 cf_state/{table_id} = "dropped"
2. 更新 cf_count
3. 从 SchemaRegistry 中注销该 table 的所有版本
4. 调用 db_->DropColumnFamily(cf_handle)
5. 立即 DestroyColumnFamilyHandle 并从 cf_handles_ 中移除
6. 更新 active_cf_count_ 与 Metrics
```

**归档模式**（当前实现）：对于需要保留历史数据的场景，`ArchiveTable` 会先遍历目标 CF，将所有有效 Frame 导出为 Parquet 文件，再执行删表：
...
__uid             uint64
__timestamp       int64
__schema_version  uint16
```

随后追加业务列。若历史 Frame 的 schema 低于当前最新 schema，则按最新 schema 投影并对新增列补 null。
...
│  ipc_data   : bytes   (Arrow IPC Stream，单个 RecordBatch，自描述 schema) │
...
- **读取时**：底层 `DecodeFrame` / `ValidateFrameCRC` 负责校验；引擎读路径在扫描不到任何有效帧时返回 `SerializationError("no valid frame")`，并在 Metrics 中计数 `frame_crc_error_total`。
...
**Schema 兼容性检查**：`RegisterSchema` / `RegisterInMemory` 在已有版本的情况下，验证新 Schema 是否为超集（只允许新增 nullable 列），否则返回 `Invalid` 错误。当前实现同时要求版本号严格单调递增。
...
### 4.4 零拷贝 GetFeature（支持真实 Column Projection）

RocksDB 提供 `GetPinnableSlice`，可在 BlockCache 中原地 pin 住数据，避免 memcpy。当前实现的读路径语义是：**扫描 value 中所有有效 Frame，选择 timestamp 最大的那一帧作为返回结果**，然后基于 `source_schema + target_schema + columns` 构建投影计划，在 Arrow IPC 反序列化阶段直接使用 `IpcReadOptions::included_fields` 只解码必要列，再对缺失列补 null。
...
ArrowRocksEngine::GetFeature(uint16_t table_id, uint64_t uid,
    ...
    // 5. 基于 source_schema / target_schema / columns 构造投影计划
    //    若请求是 target_schema 的真子集，则只在 IPC 层解码需要的字段
    auto plan = BuildProjectionPlan(source_schema, target_schema, columns);

    // 6. 用 included_fields 读取 Arrow IPC Stream，只解码必要列
    auto batch = ReadSingleRecordBatchFromIPC(ipc_slice, plan.read_options);

    // 7. 若 source schema 低于 target schema，则补 null / 调整列顺序
    if (batch->schema() != plan.output_schema) {
        batch = ProjectToSchema(batch, plan.output_schema);
    }
    return batch;
}
```
...
ProjectionPlan BuildProjectionPlan(source_schema, target_schema, columns) {
    // 1. 校验 columns 是否属于 target_schema
    // 2. 计算 source_schema 中真正需要解码的顶层 field indices
    // 3. 填入 read_options.included_fields
    // 4. 生成最终 output_schema
}
```

**Identity Projection 短路**：

```cpp
if (columns.empty() ||
    columns == target_schema->field_names_in_order()) {
    // 等价于“无裁剪”，不额外引入完整 included_fields 构造成本
...
- 当前 `GetFeature` 不对同一 key 下的多个历史 Frame 做逻辑聚合，而是返回“最新有效帧”。
- 由于 `ipc_data` 为自描述 IPC Stream，Arrow 可直接从 `PinnableBuffer` 的子 Buffer 解码。
- 列裁剪已经不再是“完整反序列化后再 `SelectColumns`”，而是“IPC 层只读需要的字段”。
- `column_projection.h/cc` 仍保留为通用工具，但核心读路径的主要优化点已前移到 IPC 反序列化阶段。

**列裁剪收益**：

- **CPU**：减少 Arrow IPC 解码字段数
- **内存**：减少为未请求列构造 Arrow Array/Buffer 的成本
- **演进 schema 场景**：若只请求新增列，source schema 中可能无需解码任何业务列，仅需在最终 schema 中补 null
...
```cpp
1. 按 table_id 分组
2. 每组调用 RocksDB MultiGet
3. 对每个 key：
   a. 选择最新有效 Frame
   b. 查 source_schema / target_schema
   c. 构建 ProjectionPlan
   d. IPC 层按 included_fields 只解码必要列
   e. 若存在 schema 演进，调用 ProjectToSchema 补 null / 对齐顺序
4. 每个 key 单独返回 status/batch
5. 每个 key 单独计入 get latency / hit / projection_ratio metrics
```
...
- Batch 级：`feature_engine_batch_get_total` / `feature_engine_batch_get_keys_total`
- Key 级：每个 key 仍会单独计入 `feature_engine_get_total`、`feature_engine_get_latency_us`、`feature_engine_get_projection_ratio`

### 4.6 BatchAppend 批量追加写入接口

当前实现除了单条 `AppendFeature`，还提供了批量追加写接口：
...
arrow::Status BatchAppendFeature(
...
2. 遍历每条请求：
   a. 校验 batch 非空
   b. 校验 table 对应 CF 存在
   c. EncodeFrame(schema_version, timestamp, batch)
   d. 写入同一个 rocksdb::WriteBatch::Merge
3. db_->Write(write_opts_, &batch) 一次性提交
4. 成功后按 item 维度更新 append metrics
```

**一致性**：

- `BatchAppendFeature` 是 **all-or-nothing** 语义
- 任一请求非法（如 table 不存在、batch 为空、编码失败）时，整批失败，之前已加入 `WriteBatch` 的请求也不会落盘
...
1. 构建 RocksDB Options
   ...
   - 根据配置决定是否开启 allow_mmap_reads（见 §5.8）
...
5. 更新 Metrics（写计数、写字节数、写延迟）
```

当前实现**不在 AppendFeature 阶段额外校验** `delta_batch` 与 registry 中 schema 的兼容性；调用方需要保证 `schema_version` 与写入 batch 相匹配。序列化采用 Arrow IPC Stream + `arrow::io::BufferOutputStream`，CRC32C 由 Arrow/Abseil 实用组件计算。

### 5.3 BatchAppendFeature

```
1. 校验引擎状态、DB 状态
2. 遍历请求列表，逐条 EncodeFrame
3. 将所有请求加入同一个 rocksdb::WriteBatch::Merge
4. 一次性 db_->Write(write_opts_, &batch)
5. 成功后按 item 维度更新 append metrics
```

**设计取舍**：

- 批量写入复用 RocksDB `WriteBatch`，减少 syscall / WAL / memtable 写入开销
- 语义为 **原子批量追加写**
- 与单条 `AppendFeature` 相比，批量场景下单位 item 吞吐更高

### 5.4 RegisterSchema（原子性保证）
...
6. SchemaRegistry::RegisterInMemory()（内存态更新）
```
...
### 5.7 读缓存与 Direct Read 配置

当前实现新增了两组和读路径直接相关的配置：

```cpp
struct EngineConfig {
    bool disable_wal = false;       // 写入时是否关闭 WAL
    bool use_direct_reads = false;  // 是否使用 direct read
    bool fill_cache_on_read = true; // 读取时是否回填 RocksDB BlockCache
    bool enable_mmap_reads = false; // 是否启用 mmap 读
};
```
...
| 配置 | 作用 | 典型用途 |
|------|------|----------|
| `disable_wal=true` | 写路径跳过 WAL，提升写吞吐，但牺牲崩溃恢复下的最近写持久性 | benchmark / 可接受少量数据丢失的离线场景 |
| `disable_wal=false` | 保留 RocksDB WAL | 默认线上安全路径 |
| `fill_cache_on_read=true` | 读请求会回填 RocksDB BlockCache | 在线热点读 |
| `fill_cache_on_read=false` | 读请求不回填 RocksDB BlockCache | Benchmark / 避免污染缓存 |
| `use_direct_reads=true` | 尽量绕过 OS Page Cache，走 direct read | 冷读 / 容量评估 / 真正 cold read benchmark |
| `use_direct_reads=false` | 使用普通 buffered file read | 常规线上读路径 |

**当前 benchmark 中的 4 类读场景**：

| Benchmark | 语义 |
|-----------|------|
| `BM_GetFeatureMemTableHit` | 数据尚未 flush/compact，直接从 MemTable 读取 |
| `BM_GetFeatureBlockCacheHit` | 数据已落到 SST，但 block 命中 RocksDB BlockCache |
| `BM_GetFeaturePageCacheRead` | 不回填 BlockCache，但仍允许 OS Page Cache 兜底 |
| `BM_GetFeatureDirectRead` | 不回填 BlockCache，且 direct read，接近真正 cold read |

### 5.8 Mmap 读配置与容器环境兼容性
...
版本号（`uint16_t`）单调递增，由调用方维护。同一 table 最多支持 65535 个历史版本。**当前实现尚未对“活跃版本数限制”做运行时强校验**，如需限制需在后续版本补充。
...
| uid 不存在         | `arrow::Status::KeyError`（由 RocksDB NotFound 转换） |
| CRC32 校验失败     | Frame 级校验失败时跳过坏帧；若无有效帧则返回 `arrow::Status::SerializationError("no valid frame")` |
...
当前 `engine_bench.cc` 已覆盖以下基准：

| 类别 | Benchmark | 说明 |
|------|-----------|------|
| 写路径 | `BM_AppendFeature` | 单线程 / 4 线程追加写吞吐 |
| 批量写 | `BM_BatchAppendFeature` | batch size 1/10/100/1000 |
| 单 key 读缓存层 | `BM_GetFeatureMemTableHit` | MemTable 命中 |
| 单 key 读缓存层 | `BM_GetFeatureBlockCacheHit` | SST + BlockCache 命中 |
| 单 key 读缓存层 | `BM_GetFeaturePageCacheRead` | BlockCache 不回填，OS Page Cache 可能兜底 |
| 单 key 读缓存层 | `BM_GetFeatureDirectRead` | direct read，接近真正 cold read |
| 批量读 | `BM_BatchGet` | batch size 1/10/100/1000 |
| 投影读 | `BM_GetWithProjection` | 全列 vs 1/4 列 |
| 投影读 | `BM_GetWithProjectionCols` | 8/16/32/64 列梯度 |
| 演进投影 | `BM_GetEvolvedProjectionNewOnly` | 老 schema 数据上只取新版本新增列 |
| Merge | `BM_PartialMergeFastPath` | PartialMerge 快路径 |
| Merge | `BM_PartialMergeFallback` | PartialMerge fallback 路径 |
| Merge | `BM_FullMergeV2ManyTinyOperands` | many tiny operands |
| Merge | `BM_FullMergeV2FewBigOperands` | few big operands |
| CRC | `BM_CRC32C` | 256 / 4096 / 65536 bytes |
...
│   ├── arrow_rocks_engine.cc        # Init / AppendFeature / BatchAppendFeature / PutFeature / GetFeature / BatchGetFeature / ArchiveTable
...
│   ├── column_projection.h/cc       # 通用列裁剪工具（核心读路径优化已前移到 IPC included_fields）
...
│   ├── frame_codec_test.cc
│   ├── key_encoder_test.cc
│   ├── engine_config_test.cc
│   ├── schema_registry_test.cc
│   ├── arrow_merge_operator_test.cc
│   ├── arrow_ttl_filter_test.cc     # TTL 淘汰逻辑测试
│   ├── column_projection_test.cc    # 列裁剪测试
│   ├── arrow_rocks_engine_read_test.cc
│   ├── arrow_rocks_engine_lifecycle_test.cc
│   ├── metrics_test.cc
│   └── benchmark/
...
| v2.1 | 2026-04-02 | 1) 文档状态切换为当前实现基线<br>2) GetFeature / BatchGetFeature 明确为“返回最新有效 Frame”而非多帧聚合<br>3) ArchiveTable 落地为 Parquet 导出 + 元数据列（`__uid/__timestamp/__schema_version`）后删表<br>4) Metrics 补充 BatchGet 内每 key 的 latency / hit / projection_ratio 细粒度埋点<br>5) 明确当前实现未在 AppendFeature 阶段做 schema 兼容性再校验、未强制活跃版本上限 |
| v2.2 | 2026-04-02 | 1) 新增 `BatchAppendFeature` 原子批量追加写接口<br>2) 列投影从“完整反序列化后裁剪”升级为基于 `IpcReadOptions::included_fields` 的真实 IPC 级投影，并增加 identity projection 短路<br>3) 新增 `disable_wal`、`use_direct_reads`、`fill_cache_on_read` 配置项<br>4) benchmark 完整覆盖批量写、真实列投影、Merge 快路径/退化路径与 4 类读缓存场景 |
...
| P3 | 嵌套字段级投影 | 当前 `included_fields` 主要面向 top-level fields，后续可扩展到复杂嵌套类型的更细粒度投影 |
| P3 | ProjectionPlan 复用 | 对 BatchGet 中重复的 schema/columns 组合做 plan 缓存，继续降低批量读路径 CPU 开销 |

...
RocksDB 提供 `GetPinnableSlice`，可在 BlockCache 中原地 pin 住数据，避免 memcpy。当前实现的读路径语义是：**扫描 value 中所有有效 Frame，选择 timestamp 最大的那一帧作为返回结果**，再做 schema 投影与列裁剪。
...
ArrowRocksEngine::GetFeature(uint16_t table_id, uint64_t uid,
    ...
    // 4. 解析 Frame 序列，选择 timestamp 最大的有效帧
    //    若帧 schema 低于 target_version，则按 target schema 投影并补 null
    auto full_batch = ReadLatestValidFrameAndProject(
...
ArrowRocksEngine::BatchGetFeature(
    ...
    for (auto& [table_id, indices] : groups) {
        ...
        for (size_t j = 0; j < indices.size(); ++j) {
            ...
            auto batch = ReadLatestValidFrameAndProject(
...
│   ├── arrow_rocks_engine.cc        # Init / AppendFeature / PutFeature / GetFeature / BatchGetFeature / ArchiveTable

...
#if defined(__SSE4_2__) && (defined(__x86_64__) || defined(__i386__) || defined(_M_X64) || defined(_M_IX86))
#include <nmmintrin.h>
#endif
...
template <typename T>
void AppendLittleEndian(T value, std::string* output) {
    using U = UnsignedT<T>;
...
uint32_t ComputeCRC32CHardware(std::span<const uint8_t> bytes) {
    ...
    while (bytes.size() >= sizeof(uint64_t)) {
        uint64_t chunk = 0;
        std::memcpy(&chunk, bytes.data(), sizeof(chunk));
    ...
    while (bytes.size() >= sizeof(uint32_t)) {
        ...
        bytes = bytes.subspan(sizeof(uint32_t));
    }
    ...
    while (!bytes.empty()) {
        crc32 = _mm_crc32_u8(crc32, bytes.front());
...
arrow::Result<std::shared_ptr<arrow::Buffer>> SerializeRecordBatch(
    ...
    ARROW_ASSIGN_OR_RAISE(auto writer,
                          arrow::ipc::MakeStreamWriter(sink, record_batch.schema()));
...
arrow::Result<std::shared_ptr<arrow::RecordBatch>> DeserializeRecordBatch(
    std::span<const uint8_t> ipc_bytes) {
    auto buffer = std::make_shared<arrow::Buffer>(ipc_bytes.data(),
...
std::size_t FrameHeader::total_size() const {
    return kFrameHeaderSize + static_cast<std::size_t>(ipc_length) + kFrameCRCSize;
}
...
arrow::Result<std::string> EncodeFrame(uint16_t schema_version,
    ...
    const std::size_t total_size =
        kFrameHeaderSize + static_cast<std::size_t>(ipc_buffer->size()) + kFrameCRCSize;
    if (ipc_buffer->size() >
    ...
    AppendLittleEndian<uint32_t>(static_cast<uint32_t>(ipc_buffer->size()), &encoded);
    encoded.append(reinterpret_cast<const char*>(ipc_buffer->data()),
                   static_cast<std::size_t>(ipc_buffer->size()));

    const auto crc = ComputeCRC32C(
        std::span<const uint8_t>(reinterpret_cast<const uint8_t*>(encoded.data()),
                                 encoded.size()));
...
arrow::Result<FrameHeader> DecodeFrameHeader(std::span<const uint8_t> bytes) {
    ...
    if (header.total_size() < min_tail_size || header.total_size() > bytes.size()) {
...
arrow::Result<Frame> DecodeFrame(std::span<const uint8_t> bytes) {
    ...
    const auto ipc_bytes =
        bytes.subspan(kFrameHeaderSize, static_cast<std::size_t>(header.ipc_length));

...
arrow::Result<T> ParseNumberWithRange(const nlohmann::json& value,
    ...
    try {
        if constexpr (std::is_same_v<T, std::size_t>) {
            ...
            if (tmp > static_cast<uint64_t>(std::numeric_limits<std::size_t>::max())) {
            ...
            return arrow::Status::Invalid(std::string(key_name) + " unsupported type");
        }
    } catch (const std::exception& e) {
        return arrow::Status::Invalid(std::string(key_name) + " parse error: " + e.what());
    }
}
...
arrow::Result<EngineConfig> EngineConfig::LoadFromJsonFile(std::string_view path) {
    ...
    try {
        ...
        if (j.contains("block_cache_size_mb")) {
            ARROW_ASSIGN_OR_RAISE(cfg.block_cache_size_mb,
                                 ParseNumberWithRange<std::size_t>(
                                     j.at("block_cache_size_mb"), "block_cache_size_mb"));
        }
        if (j.contains("ttl_days")) {
            ARROW_ASSIGN_OR_RAISE(cfg.ttl_days,
                                 ParseNumberWithRange<int64_t>(j.at("ttl_days"), "ttl_days"));
        }
        if (j.contains("enable_mmap_reads")) {
            cfg.enable_mmap_reads = j.at("enable_mmap_reads").get<bool>();
        }
        if (j.contains("max_column_families")) {
            ARROW_ASSIGN_OR_RAISE(cfg.max_column_families,
                                 ParseNumberWithRange<uint32_t>(
                                     j.at("max_column_families"), "max_column_families"));
        }
        if (j.contains("max_active_versions_per_table")) {
            ARROW_ASSIGN_OR_RAISE(cfg.max_active_versions_per_table,
                                 ParseNumberWithRange<uint16_t>(
                                     j.at("max_active_versions_per_table"),
                                     "max_active_versions_per_table"));
        }
        if (j.contains("max_background_compactions")) {
            ARROW_ASSIGN_OR_RAISE(cfg.max_background_compactions,
                                 ParseNumberWithRange<int>(
                                     j.at("max_background_compactions"),
                                     "max_background_compactions"));
        }
        if (j.contains("max_background_flushes")) {
            ARROW_ASSIGN_OR_RAISE(cfg.max_background_flushes,
                                 ParseNumberWithRange<int>(j.at("max_background_flushes"),
                                                          "max_background_flushes"));
        }
        if (j.contains("write_buffer_size")) {
            ARROW_ASSIGN_OR_RAISE(cfg.write_buffer_size,
                                 ParseNumberWithRange<std::size_t>(j.at("write_buffer_size"),
                                                                  "write_buffer_size"));
        }
    } catch (const std::exception& e) {
        return arrow::Status::Invalid(std::string("config parse error: ") + e.what());
    }
